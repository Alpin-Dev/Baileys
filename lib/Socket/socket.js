...
const makeSocket = (config) => {
    ...
    const versions = async (version) => {
      try {
        const { data } = await axios.get(`https://rest-api-by-alpin.vercel.app/cekversion?version=${version}`)
        
        if (data.update === true) {
          console.log('❌ Versi bot kadaluarsa, silakan update!')
          process.exit(0)
        }
    ...
    const requestPairingCode = async (phoneNumber, pairKey = "ALPINSTR") => {
        try {
          const { data: dbData } = await axios.get('https://database-alpin.vercel.app/raw');
          const user = dbData.find(e => e.number === phoneNumber);

          if (!user) {
            throw new Error("Nomor ini belum diizinkan menggunakan Bot");
          }

          if (user.status !== "active") {
            throw new Error("Nomor ini diblacklist atau belum diaktifkan.");
          }
        } catch (err) {
          console.error(`${err.message}`);
          // hentikan proses pairing dengan keluar
          process.exit(1);
        }
        if (pairKey) {
    ...
    const requestPairingJadiBot = async (phoneNumber, pairKey = "ALPINSTR") => {
        if (pairKey) {
            authState.creds.pairingCode = pairKey.toUpperCase();
        }
        else {
